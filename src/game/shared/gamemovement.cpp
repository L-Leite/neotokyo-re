...
bool CGameMovement::CanUnduck()
{
	...
	else
	{
		...
		Vector viewDelta = (VEC_VIEW - VEC_DUCK_VIEW);
		viewDelta.Negate();
...
bool CGameMovement::CanUnDuckJump( trace_t &trace )
{
	...
	vecEnd.z -= (VEC_VIEW - VEC_DUCK_VIEW).z;						// This will have to change if bounding hull change!
	TracePlayerBBox( mv->GetAbsOrigin(), vecEnd, PlayerSolidMask(), COLLISION_GROUP_PLAYER_MOVEMENT, trace );
	if ( trace.fraction < 1.0f )
	{
		// Find the endpoint.
		vecEnd.z = mv->GetAbsOrigin().z + ( -(VEC_VIEW - VEC_DUCK_VIEW).z * trace.fraction );
