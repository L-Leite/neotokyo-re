...
IMPLEMENT_NETWORKCLASS_ALIASED( WeaponNEOMelee, DT_WeaponNEOMelee )

BEGIN_NETWORK_TABLE( CWeaponNEOMelee, DT_WeaponNEOMelee )
END_NETWORK_TABLE()

BEGIN_PREDICTION_DATA( CWeaponNEOMelee )
END_PREDICTION_DATA()
...
// Constructor
//-----------------------------------------------------------------------------
CWeaponNEOMelee::CWeaponNEOMelee()
{
...
//-----------------------------------------------------------------------------
void CWeaponNEOMelee::Spawn( void )
{
	m_fMinRange1	= 0;
	m_fMinRange2	= 0;
	m_fMaxRange1	= 64;
...
//-----------------------------------------------------------------------------
void CWeaponNEOMelee::Precache( void )
{
	//Call base class first
	BaseClass::Precache();
}
...
//------------------------------------------------------------------------------
void CWeaponNEOMelee::ItemPostFrame( void )
{
...
// Output  :
//------------------------------------------------------------------------------
void CWeaponNEOMelee::PrimaryAttack()
{
	
#ifndef CLIENT_DLL
	CNEOPlayer *pPlayer = ToNEOPlayer( GetPlayerOwner() );
...
// Output  :
//------------------------------------------------------------------------------
void CWeaponNEOMelee::SecondaryAttack()
{
	Swing( true );
}
...
//------------------------------------------------------------------------------
void CWeaponNEOMelee::Hit( trace_t &traceHit, Activity nHitActivity )
{
	CNEOPlayer *pPlayer = ToNEOPlayer( GetOwner() );
...
Activity CWeaponNEOMelee::ChooseIntersectionPointAndActivity( trace_t &hitTrace, const Vector &mins, const Vector &maxs, CNEOPlayer *pOwner )
{
	int			i, j, k;
	float		distance;
...
// Input  : &traceHit - 
//-----------------------------------------------------------------------------
bool CWeaponNEOMelee::ImpactWater( const Vector &start, const Vector &end )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponNEOMelee::ImpactEffect( trace_t &traceHit )
{
...
//------------------------------------------------------------------------------
void CWeaponNEOMelee::Swing( int bIsSecondary )
{
	trace_t traceHit;

	// Try a ray
	CNEOPlayer *pOwner = ToNEOPlayer( GetOwner() );
	if ( !pOwner )
		return;
