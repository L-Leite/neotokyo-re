...
IMPLEMENT_NETWORKCLASS_ALIASED( BaseNEOGrenade, DT_BaseNEOGrenade )

BEGIN_NETWORK_TABLE(CBaseNEOGrenade, DT_BaseNEOGrenade)
...
#ifdef CLIENT_DLL
BEGIN_PREDICTION_DATA( CBaseNEOGrenade )
	DEFINE_PRED_FIELD( m_bRedraw, FIELD_BOOLEAN, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( m_bRedraw, FIELD_BOOLEAN, FTYPEDESC_INSENDTABLE ),
END_PREDICTION_DATA()
#endif

LINK_ENTITY_TO_CLASS( weapon_baseneogrenade, CBaseNEOGrenade );


CBaseNEOGrenade::CBaseNEOGrenade()
{
	m_bRedraw = false;
	m_bPinPulled = false;
	m_fThrowTime = 0;
...
//-----------------------------------------------------------------------------
bool CBaseNEOGrenade::Holster( CBaseCombatWeapon *pSwitchingTo )
{
	m_bRedraw = false;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseNEOGrenade::PrimaryAttack()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseNEOGrenade::SecondaryAttack()
{
	if ( m_bRedraw )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseNEOGrenade::ItemPostFrame()
{
	CNEOPlayer *pPlayer = GetPlayerOwner();
	if ( !pPlayer )
		return;
	...
	void CBaseNEOGrenade::DecrementAmmo( CBaseCombatCharacter *pOwner )
	{
	}

	void CBaseNEOGrenade::DropGrenade()
	{
		m_bRedraw = true;
		m_fThrowTime = 0.0f;
	}

	void CBaseNEOGrenade::ThrowGrenade()
	{
		m_bRedraw = true;
		m_fThrowTime = 0.0f;
	}

	void CBaseNEOGrenade::StartGrenadeThrow()
	{
	...
	int CBaseNEOGrenade::CapabilitiesGet()
	{
	...
	void CBaseNEOGrenade::EmitGrenade( Vector vecSrc, QAngle vecAngles, Vector vecVel, AngularImpulse angImpulse, CBasePlayer *pPlayer )
	{
		Assert( 0 && "CBaseNEOGrenade::EmitGrenade should not be called. Make sure to implement this in your subclass!\n" );
	}

	bool CBaseNEOGrenade::AllowsAutoSwitchFrom( void ) const
	{
		return !m_bPinPulled;
	}
