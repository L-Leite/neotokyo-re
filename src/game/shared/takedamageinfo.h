...
#ifdef NEO_DLL
	bool			IsHeadshot() const					{ return m_bHeadShot; }
	void			SetHeadshot( bool val )				{ m_bHeadShot = val;  }

	int				GetHitLocation() const					{ return m_iHitLocation; }
	void			SetHitLocation( int val )				{ m_iHitLocation = val;  }

	bool			GetUnknown() const { return m_bUnknown; }
	void			SetUnknown( bool val ) { m_bUnknown = val; }
#endif
