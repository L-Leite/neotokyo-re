...
class CNEOBot : public CNEOPlayer
{
public:
	bool			m_bBackwards;
...
public:
	static CBasePlayer* ClientPutInServerOverride_Bot( edict_t *pEdict, const char *playername )
	{
		...
		CNEOBot *pPlayer = static_cast<CNEOBot *>( CreateEntityByName( "neo_bot" ) );
		if ( pPlayer )
		{
...
CBasePlayer *BotPutInServer( bool bFrozen )
{
	...
	// This trick lets us create a CNEOBot for this client instead of the CNEOPlayer
	...
	CNEOBot *pPlayer = ((CNEOBot*)CBaseEntity::Instance( pEdict ));
...
void Bot_RunAll( void )
{
	...
	{
		CNEOPlayer *pPlayer = ToNEOPlayer( UTIL_PlayerByIndex( i ) );
...
// Output : 	virtual void
//-----------------------------------------------------------------------------
static void RunPlayerMove( CNEOPlayer *fakeclient, CUserCmd &cmd, float frametime )
{
	if ( !fakeclient )
		return;
...
void Bot_UpdateStrafing( CNEOBot *pBot, CUserCmd &cmd )
{
...
void Bot_UpdateDirection( CNEOBot *pBot )
{
...
void Bot_FlipOut( CNEOBot *pBot, CUserCmd &cmd )
{
...
void Bot_HandleSendCmd( CNEOBot *pBot )
{
	if ( strlen( bot_sendcmd.GetString() ) > 0 )
	{
		//send the cmd from this bot
		CCommand botCmd;
		botCmd.Tokenize( bot_sendcmd.GetString() );
		pBot->ClientCommand( botCmd );
...
void Bot_ForceFireWeapon( CNEOBot *pBot, CUserCmd &cmd )
{
...
void Bot_SetForwardMovement( CNEOBot *pBot, CUserCmd &cmd )
{
...
void Bot_HandleRespawn( CNEOBot *pBot, CUserCmd &cmd )
{
