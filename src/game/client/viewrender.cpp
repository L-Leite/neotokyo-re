...
#endif
#if defined( HL2_CLIENT_DLL ) || defined( CSTRIKE_DLL )	|| defined( NEO_DLL )
#define USE_MONITORS
#endif
...
int g_iLocalPlayerVision = VISION_NONE;

ConVar mat_R8Bloom( "mat_R8Bloom", "0" );
ConVar mat_R8BloomGamma( "mat_R8BloomGamma", "1.0" );
ConVar mat_R8BloomLvl( "mat_R8BloomLvl", "1.0" );
ConVar mat_R8Illustration( "mat_R8Illustration", "0", FCVAR_CHEAT );
#endif
...
#ifdef NEO_DLL
	CLIENTEFFECT_MATERIAL( "dev/camfx_brightpass" )
	CLIENTEFFECT_MATERIAL( "dev/camfx_blur" )
	CLIENTEFFECT_MATERIAL( "dev/camfx_camera" )
	CLIENTEFFECT_MATERIAL( "dev/nightvision_brightpass" )
	CLIENTEFFECT_MATERIAL( "dev/nightvision_nv" )
	CLIENTEFFECT_MATERIAL( "dev/neo_gaussx" )
	CLIENTEFFECT_MATERIAL( "dev/neo_gaussy" )
	CLIENTEFFECT_MATERIAL( "dev/thermalvision_tv2" )
	CLIENTEFFECT_MATERIAL( "dev/thermalvision_tv" )
	CLIENTEFFECT_MATERIAL( "dev/motionvision" )
...
#ifdef NEO_DLL	 
void CViewRender::DrawNightvisionOverlay()
{		   
	if ( !g_pMaterialSystemHardwareConfig->SupportsPixelShaders_2_0() )
		return;

	IMaterial* pMatNightBright = materials->FindMaterial( "dev/nightvision_brightpass", TEXTURE_GROUP_OTHER );

	if ( IsErrorMaterial( pMatNightBright ) )
	{
		DevMsg( "Couldn't find \"dev/nightvision_brightpass\" for Nightvision overlay!\n" );
		return;
	}

	IMaterial* pCamBlur = materials->FindMaterial( "dev/camfx_blur", TEXTURE_GROUP_OTHER );

	if ( IsErrorMaterial( pCamBlur ) )
	{
		DevMsg( "Couldn't find \"dev/camfx_blur\" for Nightvision overlay!\n" );
		return;
	}

	IMaterial* pMatNightvision = materials->FindMaterial( "dev/nightvision_nv", TEXTURE_GROUP_OTHER );

	if ( IsErrorMaterial( pMatNightvision ) )
	{
		DevMsg( "Couldn't find \"dev/nightvision_nv\" for Nightvision overlay!\n" );
		return;
	}
	...
	UpdateScreenEffectTexture( 1, x, y, w, h );

	int smallHdrWidth = GetSmallBufferHDR0()->GetActualWidth();
	int smallHdrHeight = GetSmallBufferHDR0()->GetActualHeight();

	pRenderContext->SetRenderTarget( GetSmallBufferHDR0() );
	pRenderContext->Viewport( 0, 0, smallHdrWidth, smallHdrHeight );
	pRenderContext->DrawScreenSpaceQuad( pRenderContext->GetCurrentMaterial() );	// Couldn't tell which material is in ida, yolo

	pRenderContext->SetRenderTarget( GetSmallBufferHDR1() );
	pRenderContext->Viewport( 0, 0, smallHdrWidth, smallHdrHeight );

	bool bFound = false;
	IMaterialVar* pBlurTexture = pCamBlur->FindVar( "$blurtexture", &bFound );

	if ( bFound )
	{
		pBlurTexture->SetTextureValue( GetSmallBufferHDR0() );	
		pCamBlur->FindVar( "$pixelsizex", &bFound )->SetFloatValue( 1.0f / smallHdrWidth );
		pCamBlur->FindVar( "$pixelsizey", &bFound )->SetFloatValue( 1.0f / smallHdrHeight );
		pCamBlur->FindVar( "$iteration", &bFound )->SetFloatValue( 1.0f );
		pRenderContext->DrawScreenSpaceQuad( pCamBlur );
	}

	pRenderContext->SetRenderTarget( GetSmallBufferHDR0() );
	pRenderContext->Viewport( 0, 0, smallHdrWidth, smallHdrHeight );

	pBlurTexture = pCamBlur->FindVar( "$blurtexture", &bFound );

	if ( bFound )
	{
		pBlurTexture->SetTextureValue( GetSmallBufferHDR1() );
		pCamBlur->FindVar( "$iteration", &bFound )->SetFloatValue( 2.0f );
		pRenderContext->DrawScreenSpaceQuad( pCamBlur );
	}
	...
	{
		pBlurTexture->SetTextureValue( GetSmallBufferHDR0() );
		pCamBlur->FindVar( "$iteration", &bFound )->SetFloatValue( 3.0f );
		pRenderContext->DrawScreenSpaceQuad( pCamBlur );
	}
	...
	{
		pBlurTexture->SetTextureValue( GetSmallBufferHDR1() );
		pCamBlur->FindVar( "$iteration", &bFound )->SetFloatValue( 4.0f );
		pRenderContext->DrawScreenSpaceQuad( pCamBlur );
	}
	...
	{
		pBlurTexture->SetTextureValue( GetSmallBufferHDR0() );
		pCamBlur->FindVar( "$iteration", &bFound )->SetFloatValue( 5.0f );
		pRenderContext->DrawScreenSpaceQuad( pCamBlur );
	}
	...
	if ( bFound )
	{
		pBlurTexture->SetMaterialValue( pMatNightBright );
		pCamBlur->FindVar( "$iteration", &bFound )->SetFloatValue( 6.0f );
		pRenderContext->DrawScreenSpaceQuad( pCamBlur );
	}
	...
	// I might be getting material vars from the wrong material
	IMaterialVar* pBloomLvl = pCamBlur->FindVar( "$bloomlvl", &bFound );

	if ( bFound )
		pBloomLvl->SetFloatValue( 10.0f );

	IMaterialVar* pFinalGamma = pCamBlur->FindVar( "$finalgamma", &bFound );

	if ( bFound )
		pFinalGamma->SetFloatValue( 0.5f );

	pRenderContext->DrawScreenSpaceQuad( pMatNightvision );
}

void CViewRender::DrawThermalOverlay()
{
	if ( !g_pMaterialSystemHardwareConfig->SupportsPixelShaders_2_0() )
		return;
	...
	if ( IsErrorMaterial( pMatNightBright ) )
	{
		DevMsg( "Couldn't find \"dev/nightvision_brightpass\" for Thermal overlay!\n" );
		return;
	}

	IMaterial* pGaussX = materials->FindMaterial( "dev/neo_gaussx", TEXTURE_GROUP_OTHER );

	if ( IsErrorMaterial( pGaussX ) )
	{
		DevMsg( "Couldn't find \"dev/neo_gaussx\" for Thermal overlay!\n" );
		return;
	}

	IMaterial* pGaussY = materials->FindMaterial( "dev/neo_gaussy", TEXTURE_GROUP_OTHER );

	if ( IsErrorMaterial( pGaussY ) )
	{
		DevMsg( "Couldn't find \"dev/neo_gaussy\" for Thermal overlay!\n" );
		return;
	}

	IMaterial* pThermalVision = materials->FindMaterial( "dev/thermalvision_tv2", TEXTURE_GROUP_OTHER );

	if ( IsErrorMaterial( pThermalVision ) )
	{
		DevMsg( "Couldn't find \"dev/thermalvision_tv2\" for Thermal overlay!\n" );
		return;
	}
	...
	ITexture* pOldRenderTarget = pRenderContext->GetRenderTarget();
	...
	bool bFound = false;
	IMaterialVar* pFbTexture = pGaussX->FindVar( "$fbtexture", &bFound );

	if ( bFound )
	{
		pFbTexture->SetTextureValue( GetSmallBufferHDR0() );
		pGaussX->FindVar( "$pixelsizex", &bFound )->SetFloatValue( 1.0f / smallHdrWidth );
		pRenderContext->DrawScreenSpaceQuad( pGaussX );
	}

	IMaterialVar* pFbTexture2 = pGaussY->FindVar( "$fbtexture", &bFound );

	if ( bFound )
	{
		pFbTexture2->SetTextureValue( GetSmallBufferHDR1() );
		pGaussY->FindVar( "$pixelsizey", &bFound )->SetFloatValue( 1.0f / smallHdrHeight );
		pRenderContext->DrawScreenSpaceQuad( pGaussY );
	}

	pRenderContext->SetRenderTarget( pOldRenderTarget );
	pRenderContext->Viewport( x, y, w, h );	   	
	pRenderContext->DrawScreenSpaceQuad( pThermalVision );
}

void CViewRender::DrawMotionOverlay()
{
	if ( !g_pMaterialSystemHardwareConfig->SupportsPixelShaders_2_0() )
		return;
	...
	if ( IsErrorMaterial( pMatNightBright ) )
	{
		DevMsg( "Couldn't find \"dev/nightvision_brightpass\" for Motion overlay!\n" );
		return;
	}
	...
	if ( IsErrorMaterial( pGaussX ) )
	{
		DevMsg( "Couldn't find \"dev/neo_gaussx\" for Motion overlay!\n" );
		return;
	}
	...
	if ( IsErrorMaterial( pGaussY ) )
	{
		DevMsg( "Couldn't find \"dev/neo_gaussy\" for Motion overlay!\n" );
		return;
	}

	IMaterial* pMotionVision = materials->FindMaterial( "dev/motionvision", TEXTURE_GROUP_OTHER );

	if ( IsErrorMaterial( pMotionVision ) )
	{
		DevMsg( "Couldn't find \"dev/thermalvision_tv2\" for Motion overlay!\n" );
		return;
	}
	...
	pRenderContext->SetRenderTarget( pOldRenderTarget );
	pRenderContext->Viewport( x, y, w, h );
	pRenderContext->DrawScreenSpaceQuad( pMotionVision );
}

void CViewRender::PerformBloomEffect()
{
	if ( !g_pMaterialSystemHardwareConfig->SupportsPixelShaders_2_0() )
		return;

	IMaterial* pCamFxBright = materials->FindMaterial( "dev/camfx_brightpass", TEXTURE_GROUP_OTHER );

	if ( IsErrorMaterial( pCamFxBright ) )
	{
		DevMsg( "Couldn't find \"dev/camfx_brightpass\" for bloom effect!\n" );
		return;
	}

	IMaterial* pCamFxBlur = materials->FindMaterial( "dev/camfx_blur", TEXTURE_GROUP_OTHER );

	if ( IsErrorMaterial( pCamFxBlur ) )
	{
		DevMsg( "Couldn't find \"dev/camfx_blur\" for bloom effect!\n" );
		return;
	}

	IMaterial* pCamFxCamera = materials->FindMaterial( "dev/camfx_camera", TEXTURE_GROUP_OTHER );

	if ( IsErrorMaterial( pCamFxCamera ) )
	{
		DevMsg( "Couldn't find \"dev/camfx_camera\" for bloom effect!\n" );
		return;
	}
	...
	bool bFound = false;
	IMaterialVar* pBlurTexture = pCamFxBlur->FindVar( "$blurtexture", &bFound );

	if ( bFound )
	{
		pBlurTexture->SetTextureValue( GetSmallBufferHDR0() );
		pCamFxBlur->FindVar( "$pixelsizex", &bFound )->SetFloatValue( 1.0f / smallHdrWidth );
		pCamFxBlur->FindVar( "$pixelsizey", &bFound )->SetFloatValue( 1.0f / smallHdrHeight );
		pCamFxBlur->FindVar( "$iteration", &bFound )->SetFloatValue( 1.0f );
		pRenderContext->DrawScreenSpaceQuad( pCamFxBlur );
	}

	pRenderContext->SetRenderTarget( GetSmallBufferHDR0() );
	pRenderContext->Viewport( 0, 0, smallHdrWidth, smallHdrHeight );

	pBlurTexture = pCamFxBlur->FindVar( "$blurtexture", &bFound );

	if ( bFound )
	{
		pBlurTexture->SetTextureValue( GetSmallBufferHDR1() );
		pCamFxBlur->FindVar( "$iteration", &bFound )->SetFloatValue( 2.0f );
		pRenderContext->DrawScreenSpaceQuad( pCamFxBlur );
	}
	...
	{
		pBlurTexture->SetTextureValue( GetSmallBufferHDR0() );
		pCamFxBlur->FindVar( "$iteration", &bFound )->SetFloatValue( 3.0f );
		pRenderContext->DrawScreenSpaceQuad( pCamFxBlur );
	}
	...
	if ( bFound )
	{
		pBlurTexture->SetMaterialValue( pCamFxBright );
		pCamFxBlur->FindVar( "$iteration", &bFound )->SetFloatValue( 4.0f );
		pRenderContext->DrawScreenSpaceQuad( pCamFxBlur );
	}
	...
	float fBloomAmmount = mat_R8BloomLvl.GetFloat();

	if ( GetMapBloomController() )
		fBloomAmmount = GetMapBloomController()->GetBloomAmount();

	IMaterialVar* pBloomLvl = pCamFxCamera->FindVar( "$bloomlvl", &bFound );

	if ( bFound )
		pBloomLvl->SetFloatValue( fBloomAmmount );

	IMaterialVar* pFinalGamma = pCamFxCamera->FindVar( "$finalgamma", &bFound );

	if ( bFound )
		pFinalGamma->SetFloatValue( mat_R8BloomGamma.GetFloat() );

	IMaterialVar* pPixelSizeX = pCamFxCamera->FindVar( "$pixelsizex", &bFound );

	if ( bFound )
		pPixelSizeX->SetFloatValue( 1.0f / smallHdrWidth );

	IMaterialVar* pPixelSizeY = pCamFxCamera->FindVar( "$pixelsizey", &bFound );

	if ( bFound )
		pPixelSizeY->SetFloatValue( 1.0f / smallHdrHeight );

	pRenderContext->DrawScreenSpaceQuad( pCamFxCamera );
}

void CViewRender::PerformExposeEffect()
{
	if ( !g_pMaterialSystemHardwareConfig->SupportsPixelShaders_2_0() )
		return;

	IMaterial* pExpose = materials->FindMaterial( "dev/expose", TEXTURE_GROUP_OTHER );

	if ( IsErrorMaterial( pExpose ) )
	{
		DevMsg( "Couldn't find \"dev/expose\" for something overlay!\n" );
		return;
	}
	...
	pRenderContext->DrawScreenSpaceQuad( pExpose );
}
#endif
...
void CViewRender::RenderView( const CViewSetup &view, int nClearFlags, int whatToDraw )
{
	...
	else
	{
...
#ifdef NEO_DLL
		if ( g_iLocalPlayerVision > 1 )
		{
			switch ( g_iLocalPlayerVision )
			{
				case VISION_NIGHT:
		...
		else
		{
			...
			if ( mat_hdr_level.GetInt() > 0 || mat_R8Bloom.GetBool() )
				PerformBloomEffect();
			
			if ( mat_R8Illustration.GetBool() )
				PerformExposeEffect();
		}
#endif
