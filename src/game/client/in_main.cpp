...
static	kbutton_t	in_leanl;
static	kbutton_t	in_leanr;
...
void IN_LeanLUp( const CCommand &args ) { KeyUp( &in_leanl, args[1] ); }
void IN_LeanLDown( const CCommand &args ) { KeyDown( &in_leanl, args[1] ); }
void IN_LeanRUp( const CCommand &args ) { KeyUp( &in_leanr, args[1] ); }
void IN_LeanRDown( const CCommand &args ) { KeyDown( &in_leanr, args[1] ); }
...
int CInput::GetButtonBits( int bResetState )
{
	...
	CalcButtonBits( bits, IN_LEANL, s_ClearInputState, &in_leanl, bResetState );
	CalcButtonBits( bits, IN_LEANR, s_ClearInputState, &in_leanr, bResetState );
...
static ConCommand startleanl( "+leanl", IN_LeanLDown );
static ConCommand endleanl( "-leanl", IN_LeanLUp );
static ConCommand startleanr( "+leanr", IN_LeanRDown );
static ConCommand endleanr( "-leanr", IN_LeanRUp );
